...
import { GestionSocios } from './GestionSocios'; // <-- Importamos el nuevo módulo
...
export const DashboardAdmin: React.FC = () => {
  ...
  // Estado para controlar qué sección del menú se renderiza en pantalla
  const [pestanaActiva, setPestanaActiva] = useState<'resumen' | 'socios'>('resumen');
  ...
  useEffect(() => {
    ...
    const canalSuscripcion = supabase
      ...
      .on(
        ...
        () => {
          if (pestanaActiva === 'resumen') cargarMetricas();
        }
      )
      .subscribe();
  ...
  return (
    <div className="min-h-screen bg-gym-negro text-gym-blanco font-sans flex flex-col">
      ...
      <header className="bg-[#242424] border-b border-gym-gris/30 p-4 flex justify-between items-center">
        ...
        {/* Menú de pestañas centralizado en la cabecera */}
        <nav className="flex gap-2">
          <button
            onClick={() => setPestanaActiva('resumen')}
            className={`px-3 py-1.5 rounded text-sm font-medium transition cursor-pointer ${
              pestanaActiva === 'resumen' ? 'bg-gym-rojo text-gym-blanco' : 'text-gym-gris hover:text-gym-blanco'
            }`}
          >
            Resumen General
          </button>
          <button
            onClick={() => setPestanaActiva('socios')}
            className={`px-3 py-1.5 rounded text-sm font-medium transition cursor-pointer ${
              pestanaActiva === 'socios' ? 'bg-gym-rojo text-gym-blanco' : 'text-gym-gris hover:text-gym-blanco'
            }`}
          >
            Gestionar Socios
          </button>
        ...
        <div className="flex items-center gap-4">
          <span className="text-gym-gris text-sm hidden md:inline font-mono">{usuario?.email}</span>
          <button 
      ...
      <main className="flex-1 p-6 max-w-7xl w-full mx-auto">
        {pestanaActiva === 'socios' ? (
          <GestionSocios /> // <-- Si la pestaña es 'socios', renderiza el nuevo módulo
        ) : (
          /* Renderizado por defecto: Resumen General */
          <>
            <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-[#242424] p-6 rounded-lg border border-gym-gris/30 relative overflow-hidden">
                ...
                <p className="text-4xl font-extrabold mt-2 text-gym-blanco">{cargandoDatos ? '...' : asistenciasHoy}</p>
              </div>
              ...
              <div className="bg-[#242424] p-6 rounded-lg border border-gym-gris/30">
                ...
                <p className="text-4xl font-extrabold mt-2 text-gym-blanco">{cargandoDatos ? '...' : membresiasActivas}</p>
              </div>
